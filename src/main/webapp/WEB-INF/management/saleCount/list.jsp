...
<form method="post" rel="pagerForm" action="${pageContext.request.contextPath}/manage/saleCount/list.html" onsubmit="return navTabSearch(this);">
        ...
        <div class="searchBar">
            <table class="searchContent">
                <tr>
                    <td>
                        <label>店铺：</label>
                        <select class="combox" name="merchantId" ref="combox_type" refUrl="${pageContext.request.contextPath}/manage/saleCount/type.html?merchantId={value}">
                            <option value="">全部</option>
                            <c:forEach var="item" items="${merchantList}">
                                <option value="${item.id}" ${vo.merchantId eq item.id ? 'selected="selected"' : ''}>${item.storeName}</option>
                            </c:forEach>
                        </select>
                    </td>
                    <td>
                        <label>菜品：</label>
                        <select class="combox" name="type" id="combox_type">
                            <option value="">全部</option>
                    ...
                    <td>
                        <label>类别：</label>
                        <select name="keywords" class="combox">
